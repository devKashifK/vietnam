...
const getIconCollectionsList = async () => {
...
const getSearchedIcon = async (searchTerm) => {
...
export function IconProvidersSelect({ handleChange }) {
  ...
  if (isLoading) return <IconProviderSelectSkeleton />;
...
export function IconPicker({ prefix, onChangeIcon, searchQuery }) {
  ...
  const { data: searchIcons, isLoading: isLoadingSearch } = useQuery({
    queryKey: ["searchIcons", searchQuery],
    queryFn: () => getSearchedIcon(searchQuery),
    enabled: !!searchQuery, // Fetch only if searchQuery exists
  });

  // Fetch icons based on the prefix when there is no search query
  const { data: icons, isLoading: isLoadingIcons } = useQuery({
    queryKey: ["icons", prefix],
    queryFn: () => getIcons(prefix),
    enabled: !searchQuery, // Fetch only if there is no search query
  });

  const iconsToRender = searchQuery ? searchIcons : icons;

  if (isLoadingSearch || isLoadingIcons || !iconsToRender) {
